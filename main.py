...
def start(update: Update, context: CallbackContext) -> None:
    ...
    links = [
        ...
        [InlineKeyboardButton(text = "Наш WhatsApp", url = "https://wa.me/+77089003223")],
    ]
...
def speaking(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "SPEAKING"
...
""", photo = open("images/check_list/check_list.png", "rb"), reply_markup = InlineKeyboardMarkup([[back_btn]]))
...
🟢 <b>Записывайте и прослушивайте свою речь:</b> Записывайте свою речь на английском языке и послушайте ее внимательно. Обратите внимание на ошибки, произношение, грамматику и попробуйте исправить их. Это поможет вам осознать свои слабые места и работать над ними.
    ...
    # text = get_bot_messages()[context.user_data['current_section']][query.data]
...
def reading(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "READING"
...
Выберите книгу, соответствующую вашему уровню владения языком и вашим целям изучения. Помните, что регулярное чтение и практика являются ключевыми для улучшения вашего английского языка.
    ...
    # message = get_bot_messages()[context.user_data['current_section']][query.data]
...
def writing(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "WRITING"
...
def listening(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "LISTENING"
...
def ielts(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "IELTS"
...
def grammar(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "GRAMMAR"
...
def vocabulary(update: Update, context: CallbackContext):
    ...
    context.user_data['current_section'] = "VOCABULARY"
...
def main() -> None:
    ...
    updater = Updater("6064136363:AAE2nez4iMOZfSnZgq8EWvpueIAyywteH1U", persistence=persistence) #actual bot
    # updater = Updater("6197471756:AAGzg6mHwG4vyunZNwN_hH6sel1vLIgycRM", persistence=persistence) #test bot
